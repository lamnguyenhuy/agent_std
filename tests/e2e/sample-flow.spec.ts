...
test("renders the Sample Repo as the primary workspace", async ({ page }) => {
  ...
  // topBar not needed — download button is unique
  const downloadPatch = page.getByRole("button", { name: "Download Patch", exact: true })
  const downloadGenerated = page.getByRole("button", { name: "Download Generated Files", exact: true })
  await expect(downloadPatch).toBeEnabled()
  await expect(downloadGenerated).toBeEnabled()
  ...
  await expect(downloadGenerated).toBeDisabled()
